# License / cash register (connection report)

The plugin skill, diagnose script, tests, and `template/` example are MIT. **$149 is the connection report** — diagnose-gated four-host screenshots from GFB accounts — not a key to run `template/`.
...
Product: `prod_V9pfxkWxzrujfU` (MCP connection report)
...
4. Pass → connect from GFB claude.ai / Desktop / Cursor / Grok → deliver screenshots within two business days
