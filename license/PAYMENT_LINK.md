...
- SKU metadata: `mcp_oauth_connect` / `attach_trace`
- Deliverable: attach trace (four-client attempt + written path). Failures welcome. Refund if we cannot attach and cannot say why.
- Fix rounds: quoted separately after the trace — not this Payment Link.

See `FULFILLMENT.md`. Do not issue a `moc_live_` key. Do not add this SKU to gfb-rescue.
