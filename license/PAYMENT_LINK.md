...
- Deliverable: connection report (four-host screenshots after free checker passes). Not a `template/` key.

After pay: run diagnose.py on the checkout MCP URL. Pass → screenshots in two business days. Fail → quote. Do not issue a moc_live_ key. Do not add this SKU to gfb-rescue.
